...
之后, 告诉MetaData我们其实想创建我们所选择的表单在实际的SQLite中的数据库, 我们使用`create_all()`, 将指向数据库的engine实例传递给他.
    ...
    metadata.create_all(engine)   # 将会执行SQL表达式创建数据库表们
...
我们单独包含此更详细的表构造,以突出主要面向 Python 内用法的最小构造与将用于在特定后端集上发出 CREATE TABLE 表达式的最小构造之间的差异。严格的要求。

## Insert Expression 插入语句

第一个SQL执行语句我们将创建一个Insert结构, 其可以代表一个INSERT表达式. 对于其相关数据库, 下面的方法是典型的创建方式.
...
从上可知INSERT表达式命名每一个列在users表中. 这能使用`values()`方法被限制, 其显式地向INSERT里建立VALUES字段.

    ins = users.insert().values(name="jack", fullname="Jack Jones")
    str(ins)
    # 输出 'INSERT INTO users (name, fullname) VALUES (:name, :fullname)'

以上, 当values方法限制VALUES字段仅有两个列时, 我们实际传递给values的数据并没有被给予到字符串中; 作为替代的我们获得绑定了名字的参数. 实际上, 我们的数据被存储在我们的Insert结构中, 但是其只会在表达式被实际执行的时候被取出使用; 从字面值的数据组成中SQLAlchemy自动地生成其绑定参数. 我们现在可以查看这些数据通过查看编译好的表单中的表达式.
...
所以现在INSERT包大师被执行了. 在输出中我们得到位置"qmark"绑定的参数而不是"named"绑定的参数. 这是为啥? 因为当执行的时候, Connection使用SQLite的方言来帮助生成表达式; 当我们使用str()方法, 表达式不知道这个方言, 之后使用named(命名)参数回到默认值. 我们可以如下手动的查看.
...
值1是自动地由SQLite产生的, 但是只有因为我们没有在我们的Insert表达式中指定id列时才会自动生成; 反之, 我们显式指定的值将会被使用. 在任一情况下, SQLALchemy总是知道如何获得一个新的主键值, 即使获取方法在不同的数据库中不同; 每一个数据库的方言知道使用特定的步骤来确认正确的一个或多个值(**注意** `ResultProxy.inserted_primary_key`返回一个列表, 以便其支持合成主键). 在这儿的方法使用`cursor.lastrowid`来从数据库的特定方法中选择来使用INSERT.. RETURNING语法; 这些执行都是透明的.

## Execute Multiple Statements 执行多个表达式

在上面的我们的插入示例是有意地有一点绘制出来用来展示一些各种各样的构成执行语言的行为. 在通常的情况下, 一个Insert表达式经常针对发送给在Connection上的`execute()`的参数被编译, 所以这不需要对Insert使用Values关键字. 让我们再一次创建一个通用的Insert表达式并以正常方式使用他.
...
以上, 因为我们在`execute()`方法中指定所有三个列的值, 编译的Insert包含所有三个列.  Insert表达式在执行时根据我们指定的参数被编译; 如果我们指定更少的参数, Insert将会在VALUES有更少的内容.
...
当执行多个集合的参数时, 每一个字典必须有相同的键的集合; 比如, 你不能比其他有更少键在一些字典中. 这是因为Insert表达式以出现在列表中的第一个字典为原型进行编译, 并假设所有的子序列参数字典是兼容这个表达式的.
